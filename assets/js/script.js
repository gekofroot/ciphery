...
let bodyShader = document.getElementById('body-shader')
...
function raiseShader() {
  let shaderHeightCount = 100
  let shaderRaiseInterval = setInterval(()=> {
    if (shaderHeightCount < 0) {
      clearInterval(shaderRaiseInterval)
    } else {
      shaderHeightCount -= 1
      bodyShader.style.height = `${shaderHeightCount}%`
    }
  }, 2)
}

let shaderLogo = document.getElementById('shader-logo')
for (let x = 0; x < shaderLogo.children.length; x++) {
  shaderLogo.children[x].style.opacity = 0
}
let letterItems = shaderLogo.children
let logoWritten = 0
let selectedIndexes = []
setTimeout(()=> {
  let letterSpillCount = 0
  let letterSpillInterval = setInterval(()=> {
    if (letterSpillCount === shaderLogo.children.length) {
      logoWritten = 1
      selectedIndexes = []
      clearInterval(letterSpillInterval)
    } else {
      let selectedIndex = Math.floor(Math.random(1) * 7)
      if (selectedIndexes.includes(selectedIndex)) {
	do {
	  selectedIndex = Math.floor(Math.random(1) * 7)
	} while (selectedIndexes.includes(selectedIndex))
      }
      shaderLogo.children[selectedIndex].style.opacity = '100%'
...
bodyShader.addEventListener('dblclick', ()=> {
  if (logoWritten === 1) {
    logoWritten = 0
    setTimeout(()=> {
      let letterUnSpillCount = shaderLogo.children.length 
      let letterUnSpillInterval = setInterval(()=> {
	if (letterUnSpillCount === 0) {
	  raiseShader()
	  clearInterval(letterUnSpillInterval)
	} else {
	  let selectedIndex = Math.floor(Math.random(1) * 7)
	  if (selectedIndexes.includes(selectedIndex)) {
	    do {
	  ...
	  shaderLogo.children[selectedIndex].style.opacity = '0'
	  selectedIndexes.push(selectedIndex)
	  letterUnSpillCount -= 1
	}
      }, 100)
    }, 200)
  }
