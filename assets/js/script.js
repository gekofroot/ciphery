...
function squareRootFunction(inputX, inputExpressionEvaluation) {
  ...
  let decimalPointIndex = String(expressionEvaluation).indexOf('.')
  
  // insert decimal
  if (decimalPointIndex === -1) {
    if (String(expressionEvaluation).includes('.')) {
      expressionEvaluation = expressionEvaluation.slice(0) + '.' + expressionEvaluation.slice(1)
    }
  } else {
    expressionEvaluation = String(expressionEvaluation)
    expressionEvaluation = expressionEvaluation.slice(0, decimalPointIndex) + '.' + expressionEvaluation.slice(2)
    expressionEvaluation = parseFloat(expressionEvaluation)
  }
  return expressionEvaluation
}
